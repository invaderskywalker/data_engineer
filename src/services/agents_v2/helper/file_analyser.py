        ...
        try:
            ...
            for s3_key in files_s3_keys_to_read:
                try:
                    ...
                    file_type = self._determine_file_type(s3_key, filename, "")
                    ...
                    if file_type in ['txt', 'doc', 'docx', 'pdf', 'ppt', 'pptx']:
                        content = self.s3_service.download_file_as_text(s3_key)

                        if not content:
                            file_result['error'] = f"No extractable text from {file_type}"
                        else:
                        ...
                        if not df_dict:
                            ...
                            preview_text = self.s3_service.download_file_as_text(s3_key)
                            file_result['content'] = preview_text
                            file_result['analysis'] = self._analyze_tabular_data(
                                file_type, df_dict, preview_text or ""
                            )
                    else:
                        file_result['error'] = f"Could not parse {file_type.upper()} file"
                    ...
                    #     df_dict = self.s3_service.download_file_as_pd_v2(s3_key, filename=filename)
                    #     if df_dict:
                    #         file_result['content'] = content[:10000] if len(content) > 10000 else content  # Preview

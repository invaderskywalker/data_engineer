    ...
    **kwargs
    ...
    if portfolio_id is not None:
        if isinstance(portfolio_id, int):
            ...
            portfolio_ids = [int(val) for val in portfolio_id]
        else:
            raise TypeError(f"portfolio_id must be int or list[int], got {type(portfolio_id)}")
...
VIEW_PORTFOLIOS_SNAPSHOT = AgentFunction(
    name="view_portfolio_snapshot",
    description="""
    A function that returns a descriptive snapshot of portfolios for this user. 

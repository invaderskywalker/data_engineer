...
def allowed_tenants(*,dev: Iterable[int] = (),qa: Iterable[int] = (),prod: Iterable[int] = (),) -> set[int]:
    env_map = {
        "dev": dev,
        "qa": qa,
        "prod": prod,
...
class DateEncoder(json.JSONEncoder):
    def default(self, obj):
        ...
        elif isinstance(obj, timedelta):       # ← ADD THIS
            return obj.days                    # timedelta → int days
...
def sanitize(data):
    """Recursively clean NaN/Infinity/unsupported values."""
    if isinstance(data, dict):
        ...
        if math.isnan(data) or math.isinf(data):
            return None  # or "NaN" if you want a string
        return data
    return data
...
class MyJSON:
    ...
    def dumps(data, **kwargs):
        clean = sanitize(data)
        return json.dumps(clean, indent=2, cls=DateEncoder)
    
    @staticmethod
    def dump(data, f, indent=2):
        clean = sanitize(data)
        return json.dump(clean, f, indent =2, cls=DateEncoder)
    
    @staticmethod
    def dumpsV2(data, **kwargs):
        raw = json.dumps(sanitize(data), indent=2, cls=DateEncoder)
        # Replace the special __options key with a comment
        import re
        def replacer(match):
            key, value, opt_key, opts = match.groups()
            return f'"{key}": "{value}", // options: {opts}'
        raw = re.sub(r'"(\w+)": "([^"]+)",\s*"(\w+)__options": (\[[^\]]+\])', replacer, raw)
        return raw
    
    @staticmethod
    def json_to_table(data, truncate_length: int = 200) -> str:
        """
        Converts a list of dictionaries (JSON array) into a readable text table.
        Handles dynamic columns, auto widths, and truncation for large text.
        
        Args:
            data (list[dict]): List of dicts (your JSON array)
            truncate_length (int): Max chars to show per cell (default 50)

        Returns:
            str: Formatted table string (Markdown-style)
        """
        if not data or not isinstance(data, list):
            return "No data available."

        # ✅ Collect all unique keys (headers)
        headers = sorted({k for item in data for k in item.keys()})

        # ✅ Compute column widths dynamically
        col_widths = {}
        for h in headers:
            max_len = max(len(str(item.get(h, ""))) for item in data)
            col_widths[h] = min(max(max_len, len(h)), 80)  # limit very large cols

        # ✅ Format header row and separator
        ...
        for item in data:
            ...
            for h in headers:
                ...
                formatted_cells.append(f"{val:<{col_widths[h]}}")
